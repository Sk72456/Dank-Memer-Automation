...
class AutoBuy(commands.Cog):
    ...
    async def shop_buy(self, item, count=1):
        ...
        def validate(msg, after=None):
            ...
            if msg.reference is not None and msg.reference.resolved is not None:
                if msg.reference.resolved.content != 'pls shop view' and msg.reference.resolved.author.id != self.bot.user.id:
                    return False
            try:
    ...
    async def shop_sell(self):
        # Sell all sellable items to complete the "Sell items" onboarding
        # objective. The dispatcher's "Pending Confirmation" handler clicks
        # the Confirm button once Dank Memer asks for confirmation.
        await self.bot.send_cmd("shop sell all")
        self.bot.log("selling items via shop sell all", "green")
